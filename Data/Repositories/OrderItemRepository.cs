...
namespace Shop.Data.Repositories
{
    public class OrderItemRepository : IOrderItemRepository
    {
        ...
        public IEnumerable<OrderItem> getSoldThisMonth()
        {
            DateTime date = DateTime.Now.Date;
            date = date.AddMonths(-1);
            return _orderItems.Where(b => (b.CreationDate >= date) && (b.Validity == Validity.Valid)).Include(b => b.Items);
        }

        public void DeleteInvalidItemOrder()
        {
            var invalidOrder = _dbContext.OrderItem.Where(o=>o.Validity==Validity.Invalid);
            foreach (var item in invalidOrder)
            {
                _dbContext.Remove(item);
                //_dbContext.Remove(_dbContext.Order.FirstOrDefault(b=>b.BillId==GetById(item.OrderItemId).id))
            }
