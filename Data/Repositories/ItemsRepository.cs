...
namespace Shop.Data.Repositories
{
    public class ItemsRepository : IItemsRepository
    {
        ...
        public IEnumerable<Items> GetAllUnApproved()
        {
            return _items.Include(b => b.Category).Include(b => b.Seller).Where(b => b.Approved==false).OrderByDescending(b => b.QuantityOrdered).ToList();
        }
        ...
        public IEnumerable<Items> GetByLocation(string searchKey, IEnumerable<Items> inputList)
        {
            ...
            return inputList.Where(b => RemoveAccents(b.Category.Name.ToLower().Replace("-", "").Replace("_", "")).Contains(_searchKey)).ToList();
        }
        ...
        {
            return _items.Include(b => b.Category).Include(b=>b.Seller).Include(b=>b.Votes).SingleOrDefault(b => b.ItemsId == itemsId);
        }

        public Items GetByItemsSlug(string slug)
        {
            return _items.Include(b => b.Category).Include(b => b.Seller)
                .Include(b => b.Comments).ThenInclude(cmt=>cmt.User)
                .Include(b=>b.Votes).ThenInclude(v=>v.User)
                .SingleOrDefault(b => b.Slug == slug);
        }
        ...
        public void UnApproveItem(string slug)
        {
            var item = GetByItemsSlug(slug);
            item.Approved = false;
            SaveChanges();
        }

        public void ApproveItem(string slug)
        {
            var item = GetByItemsSlug(slug);
            item.Approved = true;
            SaveChanges();
        ...
        public void VoteItem(string slug, Vote vote)
        {
            var item = GetByItemsSlug(slug);
        ...
        public IEnumerable<Comment> GetCommentsBySlug(string slug)
        {
            var item = GetByItemsSlug(slug);
        ...
        public IEnumerable<Vote> GetVotesBySlug(string slug)
        {
            var item = GetByItemsSlug(slug);
            return item.Votes;
        }
