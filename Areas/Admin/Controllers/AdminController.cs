...
namespace Shop.Areas.Admin.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        {
            _orderItemRepository.DeleteInvalidItemOrder();
        ...
        [HttpGet]
        public IActionResult ItemsUnapprove()
        {
            return View(new ItemsOverviewViewModel(_itemsRepository.GetAllUnApproved()));
        }
        ...
        [HttpGet]
        public IActionResult UnapproveItem(string slug)
        {
            _itemsRepository.UnApproveItem(slug);
            return RedirectToAction("ItemsOverview");
        }

        [HttpGet]
        public IActionResult ApproveItem(string slug)
        {
            _itemsRepository.ApproveItem(slug);
            return RedirectToAction("ItemsOverview");
        }
        ...
        public IActionResult SoldItems()
        {
            ...
            var allOrder = _orderRepository.GetAllOrders();
            foreach (var item in allOrder)
            {
                if (item.Orders.Count < 1)
                    continue;
                if (item.Orders.All(bl => bl.Validity != Validity.Invalid))
                    listOrder.Add(item);
            }
            //ViewData["Order"] = listOrder;

            //return View(new OverviewSoldOrderViewModel(_orderItemRepository.getSoldOrder()));
            return View(new OverviewSoldOrderViewModel(listOrder));
        }
        ...
        public IActionResult SoldItemsView(int id)
        {
            ...
            var model = new SoldItemsViewModel();
            ...
            if (Order != null)
            {
                ...
                var listNameTemp = new List<string>();
                var detailModel = new List<Shop.Models.ManageViewModels.DetailOrderModel>();

                foreach (var item in orderItems)
                {
                    listNameTemp.Add(item.Items.Name);
                }

                listNameTemp = listNameTemp.Distinct().ToList();

                foreach (var name in listNameTemp)
                {
                    detailModel.Add(new Shop.Models.ManageViewModels.DetailOrderModel
                    {
                        Name = name,
                        Price = orderItems.First(o => o.Items.Name == name).Price,
                        Slug = orderItems.First(o => o.Items.Name == name).Items.Slug,
                        Count = orderItems.Count(o => o.Items.Name == name)
                    });
                }

                var viewModel = new SoldItemsViewModel(orderItems[0], detailModel, id);
